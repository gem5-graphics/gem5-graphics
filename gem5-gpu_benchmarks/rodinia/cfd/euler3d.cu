...
void dump(float* variables, int nel, int nelr)
{
...

	{
		...
		for(int i = 0; i < nel; i++) std::cout << h_variables[i + VAR_DENSITY*nelr] << std::endl;
	}
...

	{
		...
		for(int i = 0; i < nel; i++)
		{
			for(int j = 0; j != NDIM; j++)
				std::cout << h_variables[i + (VAR_MOMENTUM+j)*nelr] << " ";
			std::cout << std::endl;
		}
	}
	
	{
		...
		std::cout << nel << " " << nelr << std::endl;
		for(int i = 0; i < nel; i++) std::cout << h_variables[i + VAR_DENSITY_ENERGY*nelr] << std::endl;
	}
	delete[] h_variables;
}
...
void initialize_variables(int nelr, float* variables)
{
	...
	CHECK_ERROR("initialize_variables failed");
}
...
void compute_step_factor(int nelr, float* variables, float* areas, float* step_factors)
{
	...
	CHECK_ERROR("compute_step_factor failed");
}
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes)
{
	...
	CHECK_ERROR("compute_flux failed");
}
...
int main(int argc, char** argv)
{
	...
	//{
		...
		FILE *scanfile = fopen(data_file_name, "r");
	
		//file >> nel;
		fscanf(scanfile, "%d", &nel);
		...
		{
			//file >> h_areas[i];
			fscanf(scanfile, "%f", &h_areas[i]);
			for(int j = 0; j < NNB; j++)
			{
				//file >> h_elements_surrounding_elements[i + j*nelr];
				fscanf(scanfile, "%d", &h_elements_surrounding_elements[i + j*nelr]);
				...
				for(int k = 0; k < NDIM; k++)
				{
					...
					fscanf(scanfile, "%f", &h_normals[i + (j + k*NNB)*nelr]);
