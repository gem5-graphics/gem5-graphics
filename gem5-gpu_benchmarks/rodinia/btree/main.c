...
#include "kernel_gpu_cuda_wrapper.h"		// (in directory provided here)
#include "kernel_gpu_cuda_wrapper_2.h"		// (in directory provided here)
...
void
wait_for_return(FILE *file_ptr) {
	char val;
	do {
		fscanf(file_ptr, "%c", &val);
		printf("%d\n", (int)val);
	} while (val != (char)'\n');
}
		...
		char** argv ) 
{
	...
	for(cur_arg=1; cur_arg<argc; cur_arg++){
		...
		// check if -cmd
		else if(strcmp(argv[cur_arg], "-cmd")==0){
			// check if value provided
			if(argc>=cur_arg+1){
				cmd_file = argv[cur_arg+1];
				cur_arg = cur_arg+1;
				// value is not a number
			}
			// value not provided
			else{
				printf("ERROR: Missing value to -cmd parameter\n");
				return 0;
			}
		}
		// unknown
	...
	// Print configuration
	printf("Configuration used: cores = %d, file = %s\n", cores_arg, input_file);
	...
	FILE *cmd_file_pointer;
	if (cmd_file == NULL) {
		printf("Waiting for command\n");
		printf("> ");
		cmd_file_pointer = stdin;
	} else {
		printf("Reading commands from file: %s\n", cmd_file);
		cmd_file_pointer = fopen(cmd_file, "r");
		if (cmd_file_pointer == NULL) {
			perror("Failure to open command file.");
			exit(EXIT_FAILURE);
		}
	}
	while (fscanf(cmd_file_pointer, "%c", &instruction) != EOF) {
		switch (instruction) {
			// ----------------------------------------40
			// Insert
			// ----------------------------------------40
			...
			case 'i':
			{
				fscanf(cmd_file_pointer, "%d", &input);
				wait_for_return(cmd_file_pointer);
				root = insert(root, input, input);
				print_tree(root);
				break;
			}
			...
			case 'k':
			{
				...
				int count;
				fscanf(cmd_file_pointer, "%d", &count);
				if(count > 65535){
			...
			{
				int start, end;
				fscanf(cmd_file_pointer, "%d", &start);
				fscanf(cmd_file_pointer, "%d", &end);
				if(start > end){
					input = start;
					start = end;
					end = input;
			...
			case 'j':
			{
				...
				fscanf(cmd_file_pointer, "%d", &count);
				int rSize;
				fscanf(cmd_file_pointer, "%d", &rSize);
