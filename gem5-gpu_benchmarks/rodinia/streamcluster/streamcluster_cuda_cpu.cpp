	  ...
	  int pid, pthread_barrier_t* barrier)
{
  ...
  while (change/cost > 1.0*e) {
    ...
    for (i=0;i<iter && pgain_count < MAX_PGAIN;i++) {
	    x = i%numfeasible;
	    change += pgain(feasible[x], points, z, k, kmax, is_center, center_table, switch_membership, isCoordChanged,
						&serial_t, &cpu_to_gpu_t, &gpu_to_cpu_t, &alloc_t, &kernel_t, &free_t, &pgain_count);
    }		
    ...
    pthread_barrier_wait(barrier);
#endif
    if (pgain_count >= MAX_PGAIN) return (cost);
  }
  return(cost);
}
	       ...
	       int pid, pthread_barrier_t* barrier )
{
  ...
  while(1) {
	...
	((k <= kmax+2)&&(k >= kmin-2))) && pgain_count < MAX_PGAIN) {
    ...
    if (((k <= kmax)&&(k >= kmin))||((loz >= (0.999)*hiz))  || pgain_count >= MAX_PGAIN)
      { 
	break;
      }
#ifdef ENABLE_THREADS
