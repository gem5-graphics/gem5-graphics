...
int main(int argc, char *argv []){
	...
	for(int f=0; f<frames_processed; f++){
		printf("frame:%d\n", f);
		for(i=f; i<common.endoPoints*common.no_frames; i+=common.no_frames){
			printf("%d\t", common.tEndoRowLoc[i]);
		}
			printf("\n");
		for(i=f; i<common.endoPoints*common.no_frames; i+=common.no_frames){
			printf("%d\t", common.tEndoColLoc[i]);
		}
			printf("\n");
		for(i=f; i<common.epiPoints*common.no_frames; i+=common.no_frames){
			printf("%d\t", common.tEpiRowLoc[i]);
		}
			printf("\n");
		for(i=f; i<common.epiPoints*common.no_frames; i+=common.no_frames){
			printf("%d\t", common.tEpiColLoc[i]);
		}
		printf("\n");
	}
#endif
