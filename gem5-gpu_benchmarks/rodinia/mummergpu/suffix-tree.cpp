...
public:
  ...
  void printDot(const char * dotfilename)
  {
    ...
    cout << "Printing dot tree to " << dotfilename << endl;
  ...
  {
    cout << endl;
    cout << "Verifing links" << endl;
    setNodePath(m_root, "");
    int linkcount = 0;
    int err = verifyNodeSuffixLinks(m_root, linkcount);
    cout << err << " suffix link errors detected" << endl;
    cout << linkcount << " suffix links checked" << endl;
  ...
  void buildUkkonen()
  {
    ...
    if (DEBUG)
    {
      cout << "Building Ukkonen Tree for " << m_string << endl
           << "Len: " << len << endl;
    }
    ...
    if (DEBUG)
    { cout << "Phase 1 Child: "; node->printLabel(cout, m_string) << endl; }
    ...
    for (int i = 2; i <= len; i++)
    {
      ...
      { 
        ...
        cout << endl;
        cout << i << ".0 " << "Phase " << i << " adding " << next << " starting with " << startj << endl;

        string beta = substr(m_string, 1, i);
        cout << i << ".1" << " Extension 1: \"" << beta << "\" [implicit]" << endl;
      }
      ...
      for (int j = startj; j <= i; j++)
      {
        ...
        if (DEBUG)
        {
          ...
          cout << i << "." << j << " Phase " << i << " Extension " << j << ": \"" << beta << "\" bp:" << betapos << endl;

          cout << i << "." << j << "  Walking up from n:";
          node->printLabel(cout, m_string) << " nw: " << nodewalk << endl;
        }
        ...
        else
        {
          if (nodewalk)
          {
            ...
            if (DEBUG)
            {
              cout << i << "." << j << "   Adjusted nw: " << nodewalk << endl;
            }
          }
          else
          {
            ...
            if (DEBUG)
            {
              cout << i << "." << j << "   Adjusted len: " << len << endl;
            }
          }
          
          if (DEBUG)
          {
            cout << i << "." << j << "   parent bp: " << betapos <<  " n:";
          ...
          if (node->m_suffix == NULL)
          {
            ...
            if (DEBUG)
            {
              cout << i << "." << j << "   grandparent bp: " << betapos << " n:";
              node->printLabel(cout, m_string) << endl;
            }
        ...
        if (DEBUG)
        {
          cout << i << "." << j << "  Starting to walk down from bp: " << betapos << " to " << i << " n:";
          node->printLabel(cout, m_string) << endl;
        }
        ...
        if (FORCEROOT && node != m_root)
        {
          ...
          if (DEBUG)
          {
            cout << i << "." << j << " AtRoot bp: " << betapos << endl;
          }
        }
        ...
        while ((betapos <= i) && !done)
        {
          ...
          if (DEBUG)
          {
            cout << i << "." << j << "  node betapos: " << betapos << "[" << base << "] n:";
            node->printLabel(cout, m_string) << " ";
            if (child) { cout << "c: "; child->printLabel(cout, m_string); }
            cout << endl;
          }
          ...
          if (!child)
          {
            if (splitnode && betapos == splitnode->m_start)
            {
              ...
              {
                cout << i << "." << j << "   Add SL1: ";
                splitnode->m_parent->printLabel(cout, m_string) << " sl-> ";
                node->printLabel(cout, m_string) << endl;
              }
            ...
            {
              cout << i << "." << j << "   New Node: ";
              newnode->printLabel(cout, m_string) << endl;
            }
          ...
          else
          {
            ...
            if (DOJUMP)
            {
              ...
              if (mustmatch >= childlen)
              {
                ...
                if (DEBUG)
                {
                  cout << i << "." << j << "   Edge Jump by: " << childlen << " new bp: " << betapos << " np: " << nodepos << endl;
                }
              ...
              else if (mustmatch)
              {
                ...
                if (DEBUG)
                {
                  cout << i << "." << j << "   Partial Jump by: " << mustmatch << " new bp: " << betapos << " np: " << nodepos << endl;
                }
            ...
            while (nodepos <= child->m_end && betapos <= i)
            {
              ...
              if (m_string[betapos] == nodebase)
              {
                ...
                if (betapos == i+1)
                {
                  if (DEBUG)
                  {
                    cout << i << "." << j << "    Internal edge match nw: " << nodewalk << endl;
                  }
                  ...
                  if ((nodewalk == child->len(i)) && (child->m_end == len))
                  {
                    ...
                    if (DEBUG)
                    {
                      cout << i << "." << j << "    Leaf Node, Implicit Rule I Extension" << endl;
                    }
                  }
                  else
                  {
                    ...
                    if (DOPHASETRICK)
                    {
                      ...
                      if (DEBUG)
                      {
                        cout << i << "." << j << "    Implicit Extension... start next phase at " << startj << ", saved " << skip << endl;
                      }
                    ...
                    if (DOINTERNALSKIP)
                    {
                      ...
                      if (DEBUG)
                      { 
                        cout << i << "." << j << "    Implicit Extension... skipping rest of phase, saved " << skip << endl;
                      }
              ...
              {
                if (DEBUG) { cout << i << "." << j << "   Spliting "; child->printLabel(cout, m_string); }
                ...
                {
                  cout << " => ";
                  split->printLabel(cout, m_string) << " + ";
                  child->printLabel(cout, m_string) << endl;
                }
                            ...
                            int min_match_len) 
{
    ...
    *childrenTexture = NULL;
    ...
    printf(" node: %dx%d",     *width, *node_height);
    printf(" children: %dx%d ", *width, *children_height);
...
void printTreeTexture(const char * texfilename,
                      ...
                      int nodecount)
{
  cout << "Printing tree texture to " << texfilename << endl;
...

{
    ...
    cout << SuffixNode::s_nodecount << " nodes "
    << btimer.str(true, 5) << endl;

#if RENUMBER_TREE    
    cout << "  Renumbering tree... ";
    EventTime_t rtimer;
    ...
    cout << rtimer.str(true, 5) << endl;
#endif
    
    EventTime_t ftimer;
    cout << "  Flattening Tree... ";
 ...
 *refstr = strdup(S.c_str());
  *reflen = strlen(*refstr) + 1;
  ...
  cout << *reflen-3 << " bp. " << timer.str(true, 5) << endl;
}
                           ...
                           bool rc)
{
    ...
    *num_match_coords = 0;
    ...
    cout <<  "Reordering complete. " << reorder_timer.str(true, 5) << endl;
    printf(" curr_warp_padding = %d, total_reordered = %d\n", curr_warp_padding, total_reordered_qry_size);
...
void flushOutput()
{
   if (bytes_written)
   {
	...
	//memset(output_buf, 0, sizeof(output_buf));
   //cout << "expectedvisit: " << expectedvisit << endl;
   //cout << "leavesvisited: " << leavesvisited << endl;
   //cout << "leavesprinted: " << leavesprinted << endl;
}
