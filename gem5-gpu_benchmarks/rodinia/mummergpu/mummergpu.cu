...
void buildReferenceTexture(Reference* ref,
                           ...
                           Statistics* statistics)
{
    printf("Building reference texture...\n");
	...
	printf("This tree will need %d bytes on the board\n", ref->bytes_on_board);
    ...
    printf("The refstr (reordered) requires %d bytes\n", refstrsize);
...
#else
    printf("The refstr requires %d bytes\n", ref->len);
...
void loadReference(MatchContext* ctx) {
    ...
    if (!ctx->on_cpu) {
        ...
        printf("after child_hist  ref->bytes_on_board:%ld\n", ref->bytes_on_board);
...
{
	printf("\tProcessing queries %s to %s\n",
		queries->h_names[0],
...
void loadQueries(MatchContext* ctx)
{
    ...
    if (!ctx->on_cpu) {
        printf("Allocating device memory for queries... ");
                           ...
                           unsigned int* num_coords)
{
	...
	if (numQueries > 0)
	{
		...
		printf("Need %d match coords for this result array\n",
			numCoords);
	}
#endif
...
void loadResultBuffer(MatchContext* ctx)
{
	...
	printf("Allocating result array for %d queries (%d bytes) ...",
   ...
   printf("board free memory: %u total memory: %u\n",
					   ...
					   unsigned int* nextqrychar)
{
	...
	*nextqry = qry;
	*nextqrychar = qrychar;
	printf("Allocing %d bytes of host memory for %d alignments\n",  alignmentOffset * sizeof(Alignment), numAlignments);
                    ...
                    unsigned int numAlignments)
{
    ...
    printf("prepared %d matches %d alignments\n", numMatches, numAlignments);
	printf("Copying %d bytes to host memory for %d alignments\n",  numAlignments * sizeof(Alignment), numAlignments);
...
void getExactAlignments(MatchContext * ctx, ReferencePage * page, bool on_cpu)
{
    ...
    while (next_coord < last_coord)
    {
	    ...
	    printf("buffer prep time= %f\n", btime);
		deleteTimer(btimer);

        printf("Round %d: Printing results for match coords [%d-%d) of %d using %d matches and %d alignments\n",
...
//		}
    ...
    printf("Finished processing %d matches and %d potential alignments in %d rounds\n",
                ...
                ReferencePage* page)
{
    ...
    printf( "Matching queries %s - %s against ref coords %d - %d\n",
...
{
	printf("Beginning reference page %p\n", page);
...
void initReferencePages( MatchContext* ctx , int* num_pages, ReferencePage** pages_out) {
    ...
    printf("Stream will use %d pages for %d bases, page size = %d\n",
...
int matchQueries(MatchContext* ctx) {
    ...
    printf("Streaming reference pages against all queries\n");
