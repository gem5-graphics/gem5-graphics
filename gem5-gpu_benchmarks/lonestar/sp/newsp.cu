...
__global__ void
update_bias(GPUCSRGraph clauses, GPUCSRGraph vars, Edge ed, float *bias_list,
            int *bias_list_vars, int *bias_len, float *g_summag)
{
    int id = threadIdx.x + blockIdx.x * blockDim.x;
    int threads = blockDim.x * gridDim.x;
    float maxmag;
...
__global__ void
decimate(GPUCSRGraph clauses, GPUCSRGraph vars, Edge ed, int *g_bias_list_vars,
...
int
compare_float(const void *x, const void *y)
{
...
float
sort_bias_list(cub::DoubleBuffer<float> &db_bias_list,
               cub::DoubleBuffer<int> &db_bias_list_vars, int *g_bias_list_len,
               float summag, int& fixperstep)
{
    int bias_list_len;
...
{
    fprintf(stderr, "usage: %s formula.cnf MAXLITERALS [soloutfile] [partialoutfile]\n", argv[0]);
...
int main(int argc, char *argv[])
{
    ...
    char *soloutfile = "sp_sol.dat";
    if (argc > 3) {
        soloutfile = argv[3];
    }
    char *partialoutfile = "partial.cnf";
    if (argc > 3) {
        partialoutfile = argv[4];
    }
    ...
    while (converge(g_cl, g_vars, g_ed, g_max_eps)) {
        ...
        decimate <<<d2_res * nSM, 384>>>(g_cl, g_vars, g_ed,
    ...
    g_vars.to_cpu(vars);
    g_ed.to_cpu(ed);
    print_solution(soloutfile, vars);
    dump_partial(partialoutfile, cl, vars, ed);
}
