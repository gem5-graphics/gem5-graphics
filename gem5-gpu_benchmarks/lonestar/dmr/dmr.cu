...
__device__ void addneighbour(Mesh &mesh, uint3 &neigh, uint elem)
{
    ...
    if (neigh.x == INVALIDID) {
        neigh.x = elem;
        return;
    }
    if (neigh.y == INVALIDID) {
        neigh.y = elem;
        return;
    }
    if (neigh.z == INVALIDID) {
        neigh.z = elem;
        return;
    }
}
...
void refine_mesh(ShMesh &mesh)
{
    ...
    if (debug) printf("Running at residency %lu.\n", RES_REFINE);
    ...
    * (nbad.cpu_wr_ptr(true)) = 0;
    ...
    while (cnbad) {
        ...
        CUDA_SAFE_CALL(cudaThreadSynchronize());   // not needed
        printf("refine over\n");
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2 || argc > 4) {
        printf("Usage: %s basefile [maxfactor] [outputdir]\n", argv[0]);
        exit(0);
    }
