...
void initBinB(struct pb_TimerSet *timers)
{
    ...
    cudaMemcpy(dev_binb, binb, (NUM_BINS + 1) * sizeof(float), cudaMemcpyHostToDevice);
               ...
               float *dev_binb, int NUM_BINS)
{
    ...
    extern volatile __shared__ unsigned int
    warp_hists[][NUM_HISTOGRAMS]; // 640B <1k
    ...
    for (unsigned int offset = NUM_HISTOGRAMS >> 1; offset > 0; offset >>= 1) {
        for (unsigned int bin_base = 0; bin_base < NUM_BINS; bin_base += BLOCK_SIZE / (NUM_HISTOGRAMS >> 1)) {
            ...
            if (warp_index < offset && bin_base + bin_index < NUM_BINS) {
                unsigned long sum = (warp_hists[bin_base + bin_index][warp_index] +
           ...
           float* d_z_data)
{
    ...
    unsigned sharedSize = NUM_BINS * NUM_HISTOGRAMS * sizeof(unsigned);

    gen_hists <<<dimGrid, dimBlock, sharedSize>>> (histograms, d_x_data,
            d_y_data, d_z_data, NUM_SETS, NUM_ELEMENTS, dev_binb, NUM_BINS);
}
// **===-----------------------------------------------------------===**
