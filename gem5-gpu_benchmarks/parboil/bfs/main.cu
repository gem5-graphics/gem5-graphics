...
int main(int argc, char** argv)
{
    ...
    do {
        ...
        if (num_of_blocks == 1) {
            ...
            BFS_in_GPU_kernel <<<grid, threads>>>(d_src, d_dst,
                    d_graph_nodes, d_graph_edges, d_color, d_cost, num_t,
                    tail, d_gray, k, d_overflow);

        } else if (num_of_blocks <= NUM_SM) {

            cudaMemcpy(num_td, &num_t, sizeof(int), cudaMemcpyHostToDevice);

            BFS_kernel_multi_blk_inGPU <<<grid, threads>>>(d_src, d_dst,
                    d_graph_nodes, d_graph_edges, d_color, d_cost, num_td,
                    tail, d_gray, k, switch_kd, max_nodes_per_block_d,
                    global_kt_d, d_overflow);

            cudaMemcpy(&switch_k, switch_kd, sizeof(int), cudaMemcpyDeviceToHost);
            ...
            BFS_kernel <<<grid, threads>>>(d_src, d_dst, d_graph_nodes,
                    d_graph_edges, d_color, d_cost, num_t, tail, d_gray, k,
                    d_overflow);
