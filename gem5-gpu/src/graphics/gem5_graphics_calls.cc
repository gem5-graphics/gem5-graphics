...
#define CALL_GSERIALIZE \
    ...
    switch (gpusysno) {
        ...
        case gem5_block:
        {
            ...
            uint32_t blockT = cudaGPU==NULL? 0 : ((g_renderData.isBusy() or g_pendingCommandBuffer.valid)? 1 : 0);
            if(blockT == 1){
              //wtrDelay = 5;
            } else if(--wtrDelay > 0){
              --wtrDelay;
        ...
        {
            CALL_GSERIALIZE_CMD;
            uint32_t active = 0; //cudaGPU==NULL? 0: gpgpusimSimulationActive() ? 1 : 0;
